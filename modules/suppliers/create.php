...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($code)) $errors[] = 'Mã nhà cung cấp không được để trống';
    if (empty($name)) $errors[] = 'Tên nhà cung cấp không được để trống';
    
    // Check if code already exists
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT id FROM suppliers WHERE code = :code");
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO suppliers (code, name, contact_person, phone, email, address, tax_code, bank_account, bank_name, notes) 
...
<div class="row">
    <div class="col-12">
        <h2><i class="bi bi-plus-circle"></i> Thêm nhà cung cấp mới</h2>
    </div>
</div>
    ...
    <div class="card-body">
        <form method="POST" action="">
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="code" class="form-label required">Mã nhà cung cấp</label>
